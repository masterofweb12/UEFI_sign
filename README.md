...
Для компиляции ядра мы должны установить следующие пакеты:
```apt install libncurses-dev ncurses-bin  dwarves build-essential gcc bc bison flex libssl-dev libelf-dev dpkg-dev```.  
